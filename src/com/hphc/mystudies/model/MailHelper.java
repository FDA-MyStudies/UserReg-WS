...
public class MailHelper
{
            ...
            try
            {
                        ...
                        "true".equalsIgnoreCase(_session.getProperty("mail.smtp.starttls.enable")))
                {
                    setSession(Session.getInstance(_session.getProperties(), new Authenticator()
                    {
                        ...
                        {
                            if (_email != null && _password != null && !_email.equalsIgnoreCase("") && !_password.equalsIgnoreCase(""))
                                return new PasswordAuthentication(_email, _password);
                            else
                                return new PasswordAuthentication(_session.getProperty("mail.smtp.user"),_session.getProperty("mail.smtp.password"));
                        }
                    }));
                }
            }
    ...
    public static MultipartMessage createMultipartMessage(String email, String password)
    {
        _email = email;
        _password = password;
    ...
    private static ViewMessage _createMessage(ViewMessage m, String from, @Nullable String to) throws MessagingException
    {
        ...
        if (null != to)
            m.addRecipients(Message.RecipientType.TO, createAddressArray(to));
        return m;
    }
    ...
    private static void logMessagingException(Message m, Exception e)
    {
        try
        {
            _log.log(Level.WARN, ERROR_MESSAGE +
                    ...
                    "to: " + StringUtils.join(m.getRecipients(Message.RecipientType.TO), "; ") + "\n" +
